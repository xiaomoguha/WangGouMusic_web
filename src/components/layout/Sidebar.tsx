...
export function Sidebar() {
    ...
    return (
        <>
            {/* 桌面端侧边栏 */}
            <aside className="fixed left-0 top-0 bottom-0 z-30 hidden w-[220px] flex-col border-r bg-card/80 backdrop-blur-sm md:flex">
                {/* Logo */}
            ...
            {/* 手机端底部导航栏 */}
            <nav className="fixed bottom-0 left-0 right-0 z-30 flex h-16 items-center justify-around border-t bg-card/95 backdrop-blur-md md:hidden" style={{ boxShadow: 'var(--shadow-player)' }}>
                {navItems.map(item => {
                    const isActive = location.pathname === item.path
                    return (
                        <NavLink
